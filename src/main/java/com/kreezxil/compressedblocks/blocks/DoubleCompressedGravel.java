...
public class DoubleCompressedGravel  extends Block {
	...
	public DoubleCompressedGravel(String unlocalizedName, Material materialIn, float hardness, float resistance) {
		super(materialIn);
	...
	public DoubleCompressedGravel(String name, float hardness, float resistance) {
		this(name, Material.rock, hardness, resistance);
	}
	
	public DoubleCompressedGravel(String name) {
		this(name, 12f, 60f);
	}
	...
	public Item getItemDropped(IBlockState state, Random par2Random, int fortune){
		...
		if(fortune > 0)
			return Item.getItemFromBlock(ModBlocks.CompressedGravel);
